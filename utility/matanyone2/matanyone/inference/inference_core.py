...
class InferenceCore:
    ...
    def step(self,
        ...
        # Ensure last_mask always has shape [batch, num_objects, H, W]
        # Sometimes pred_prob_with_bg[1:] is 3D [num_objects, H, W], need to add batch dim
        self.last_mask = pred_prob_with_bg[1:]
        if self.last_mask.dim() == 3:
            self.last_mask = self.last_mask.unsqueeze(0)  # Add batch dimension
        if self.flip_aug:
