...
export class WanDialog extends EventTarget {
    constructor(options) {
        super();
        this.options = options;
        let container = $el("div.wan-dialog-container");
        this.element = $el("dialog", {
            classes: ["wan-dialog", options.class || ""],
            child: container,
            ...
            events: {
                click: (event) => {
                    if (!this.element.open ||
                        ...
                        getClosestOrSelf(event.target, `.wan-dialog-container`) === container) {
                        return;
                    }
                    return this.close();
                },
        ...
        this.titleElement = $el("div.wan-dialog-container-title", {
            parent: container,
        ...
        this.contentElement = $el("div.wan-dialog-container-content", {
            parent: container,
            child: options.content,
        });
        const footerEl = $el("footer.wan-dialog-container-footer", { parent: container });
    ...
    show() {
        document.body.classList.add("wan-dialog-open");
    ...
    onDialogElementClose() {
        document.body.classList.remove("wan-dialog-open");
        this.element.remove();
