...
FIXED_LENGTH = 121
def pad_pts(tr):
    """Convert list of {x,y,v} to (FIXED_LENGTH,1,3) array, padding/truncating.
    Respects visibility flags from the input if present."""
    pts = np.array([[p['x'], p['y'], p.get('v', 1)] for p in tr], dtype=np.float32)
    n = pts.shape[0]
    if n < FIXED_LENGTH:
        # Create padding with the last point but with visibility = 0
        last_point = pts[-1].copy()
        last_point[2] = 0  # Set visibility to 0 for padding
        pad = np.array([last_point] * (FIXED_LENGTH - n), dtype=np.float32)
...
class WanVideoATITracksVisualize_v2:
    @classmethod
    def INPUT_TYPES(s):
        return {"required": {
            "images": ("IMAGE",),
...
NODE_CLASS_MAPPINGS = {
    "WanVideoATITracks_v2": WanVideoATITracks_v2,
    "WanVideoATITracksVisualize_v2": WanVideoATITracksVisualize_v2,
    "WanVideoATI_comfy_v2": WanVideoATI_comfy_v2,
    }
NODE_DISPLAY_NAME_MAPPINGS = {
    "WanVideoATITracks_v2": "WanVideo ATI Tracks v2",
    "WanVideoATITracksVisualize_v2": "WanVideo ATI Tracks Visualize v2",
    "WanVideoATI_comfy_v2": "WanVideo ATI Comfy v2",
    }
